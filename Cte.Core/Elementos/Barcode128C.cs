...
namespace Cte.Core.Elementos
{
    internal class Barcode128C : ElementoBase
    {
        ...
        protected virtual float MargemVerticalEfetiva => MargemVertical;
        ...
        public Barcode128C(string code, Estilo estilo, float largura = 75F) : base(estilo)
        {
            ...
            {
                throw new ArgumentException("O código deve apenas conter dígitos numéricos.", "code");
            }
        ...
        private void DrawBarcode(RectangleF rect, Gfx gfx)
        {
            ...
            for (int i = 0; i < codeBytes.Count; i++)
            {
                ...
                for (int i2 = 0; i2 < pt.Length; i2++)
                {
                    if (i2 % 2 == 0)
                    {
                        // Módulo "preto" do código de barras é preencher retângulo.
        ...
        public override void Draw(Gfx gfx)
        {
            ...
            float larguraEfetiva = Math.Min(Largura, Width);
            float w2 = (Width - larguraEfetiva) / 2F;
            float margemVertical = Math.Min(MargemVerticalEfetiva, Height / 2F);
            DrawBarcode(new RectangleF(X + w2, Y + margemVertical, larguraEfetiva, Height - 2 * margemVertical), gfx);
        }
    }
}
