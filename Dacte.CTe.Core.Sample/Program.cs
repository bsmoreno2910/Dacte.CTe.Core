...
namespace Dacte.CTe.Core.Sample
{
    /// <summary>
    ...
    public static class Program
    {
        ...
        private static void Gerar(string xmlPath, string pdfPath, string logoPath)
        {
            ...
            // Diagnóstico rápido da geração.
            Console.WriteLine($"      Modal:{modelo.Modal} Modelo:{modelo.Modelo} Série:{modelo.Serie} N{modelo.Numero}");
            ...
            Console.WriteLine($"      Rodo:{modelo.Rodoviario?.Rntrc ?? "(null)"} Aéreo:{modelo.Aereo?.NumeroOperacionalConhecimento ?? "(null)"} Multi:{modelo.Multimodal?.CertificadoOTM ?? "(null)"}");
        ...
        private static (string entrada, string saida, string logoPath) ResolverCaminhos(string[] args)
        {
            ...
            else                  logoPath = Environment.GetEnvironmentVariable("DACTE_CTE_CORE_SAMPLE_LOGO");
