...
namespace Dacte.CTe.Core.Graphics
{
    /// <summary>
    ...
    {
        /// <summary>
        /// XGraphics do PdfSharpCore associado é página.
        /// </summary>
        ...
        /// via <see cref="XStringFormat"/> é isso evita erros de centralização que
        /// surgiriam ao medir a largura em um contexto separado (CreateMeasureContext)
        /// e passá-la é <c>DrawString</c>, pois a fonte usada na medição pode
            ...
            AlinhamentoVertical av = AlinhamentoVertical.Topo)
        {
            ...
            switch (ah)
            {
                case AlinhamentoHorizontal.Centro:
                    xAnchorPoint = (rect.X + rect.Width / 2F).ToPoint();
                    xAlign = XStringAlignment.Center;
                    break;
                case AlinhamentoHorizontal.Direita:
                    xAnchorPoint = rect.Right.ToPoint();
                    xAlign = XStringAlignment.Far;
                    break;
                default:
                    xAnchorPoint = rect.X.ToPoint();
            ...
            XGraphics.DrawString(str, fonte.FonteInterna, TextBrush,
                new XPoint(xAnchorPoint, baselinePt),
                fmt);
        }

        /// <summary>
        /// Desenha a string tratando <paramref name="point"/> (mm) como top-left visual
        /// do texto é compatível com o layout original calibrado para PDFClown.
        ...
        public void ShowXObjectStretch(XImage xobj, RectangleF r)
        {
            if (xobj == null) throw new ArgumentNullException(nameof(xobj));
            CheckRectangle(r);

            XGraphics.DrawImage(xobj,
                new XRect(r.X.ToPoint(), r.Y.ToPoint(), r.Width.ToPoint(), r.Height.ToPoint()));
        }
        ...
        public void DrawRoundedRectangle(RectangleF rect, float radius)
        {
            if (radius < 0) throw new ArgumentOutOfRangeException(nameof(radius));
            CheckRectangle(rect);
            XGraphics.DrawRoundedRectangle(StrokePen,
                rect.X.ToPoint(), rect.Y.ToPoint(), rect.Width.ToPoint(), rect.Height.ToPoint(),
                radius.ToPoint(), radius.ToPoint());
        }
        ...
        /// Aplica a rotacao usando a transformacao nativa do PdfSharpCore.
        /// </summary>
        ...
        private void CheckRectangle(RectangleF r)
        {
            if (r.X < 0 || r.Y < 0 || r.Width <= 0 || r.Height <= 0)
                throw new ArgumentException($"RectangleF inválido: X={r.X:F2} Y={r.Y:F2} W={r.Width:F2} H={r.Height:F2}", nameof(r));
        }
        ...
        // Mantidos como no-op para manter a assinatura usada pelo código legado.
            // No PdfSharpCore o desenho é imediato e não há buffer explícito.
